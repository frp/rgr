...
TEST_CASE( "token types test", "[parseToken]" ) {
    ...
    SECTION( "keywords" ) {
        REQUIRE (parseToken("var").type == TokenType::var_);
...
TEST_CASE ( "lexing whole string test", "[lexString]" ) {
    ...
    {
        REQUIRE ( lexString("int a, b, c") == expected_tok_seq);
        ...
        REQUIRE ( lexString("end.") == (vector<Token> { { TokenType::endwithdot, "end.", 1 } } ));
    }
